...
public class RuralScene : MonoBehaviour
{
    public Camera cam;
    RaycastHit hit;
    public float range;
    ...
    void ChangeUIElements()
    {
        if (select1)
        {
            ...
            textToChange.text = $"Wood 100 {block1}x"; // Display block type and count
        }
        else if (select2)
        {
            imageToChange.sprite = ConcreteSprite;
            textToChange.text = $"Wood 170 {block2}x"; // Display block type and count
        }
        else if (select3)
        {
            imageToChange.sprite = WoodSprite;
            textToChange.text = $"Wood 140 {block3}x"; // Display block type and count
        }
        else
        {
