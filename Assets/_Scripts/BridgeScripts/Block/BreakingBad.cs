...
public class BreakingBad : MonoBehaviour
{
    ...
    List<GameObject> startingPointBlocks = new List<GameObject>();
    List<GameObject> endingPointBlocks = new List<GameObject>();
    GameObject startingPointBlock = null;
    GameObject newStartingPointBlock = null;
    ...
    private void BuildBlock(GameObject block)
    {
        if (Physics.Raycast(cam.transform.position, cam.transform.forward, out hit, range))
        {
            if (hit.transform.tag == "Block")
            {
                ...
                foreach (GameObject startingPointBlock in startingPointBlocks)
                {
                    if (Vector3.Distance(startingPointBlock.transform.position, hit.point) < connectionDistance)
                    {
                        newStartingPointBlock = startingPointBlock;
                        break;
                    }
                }

                if (startingPointBlock != null)
                {
                    FixedJoint startingPointBlockFixedJoint = startingPointBlock.GetComponent<FixedJoint>();
                    FixedJoint newBlockFixedJoint = block.GetComponent<FixedJoint>();

                    startingPointBlockFixedJoint.connectedBody = newBlockFixedJoint.connectedBody;
                    newBlockFixedJoint.connectedBody = startingPointBlockFixedJoint.connectedBody;

                    startingPointBlocks.Remove(startingPointBlock);
                    endingPointBlocks.Add(block);
                }
                else
                {
