...
public class NatureScene : MonoBehaviour
{
    ...
    void ChangeUIElements()
    {
        if (select1)
        {
            ...
            textToChange.text = $"Sand 5 {block1}x"; // Display block type and count
        }
        else if (select2)
        {
            imageToChange.sprite = ConcreteSprite;
            textToChange.text = $"Sand 9 {block2}x"; // Display block type and count
        }
        else if (select3)
        {   
            imageToChange.sprite = WoodSprite;
            textToChange.text = $"Sand 4 {block3}x"; // Display block type and count
        }
        else
        {
