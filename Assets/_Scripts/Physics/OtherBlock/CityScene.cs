...
public class CityScene : MonoBehaviour
{
    ...
    void ChangeUIElements()
    {
        if (select1)
        {
            ...
            textToChange.text = $"Concrete 75 {block1}x"; // Display block type and count
        }
        else if (select2)
        {
            imageToChange.sprite = ConcreteSprite;
            textToChange.text = $"Concrete 110 {block2}x"; // Display block type and count
        }
        else if (select3)
        {
            imageToChange.sprite = WoodSprite;
            textToChange.text = $"Concrete 90 {block3}x"; // Display block type and count
        }
        else
        {
